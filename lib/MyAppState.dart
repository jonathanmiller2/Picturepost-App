...
class MyAppState extends State<MyApp> {
        ...
        for (var post in posts) {
          if (posts.indexOf(post) == favorite.id) {
            favoriteMarkers.add(new MapMarker(
                    ...
                    icon: BitmapDescriptor.fromAsset("assets/favorite.png"),
                    name: post.name,
                    ...
                    setId: post.referencePictureSetId)
                .toMarker());
          }
        }
      }
      for (var favorite in favoriteMarkers) {
        allMarkers.removeWhere((element) => int.parse(element.markerId.value) == int.parse(favorite.markerId.value));
      }
      for (var favorite in favoriteMarkers) {
    ...
    if (favorites.length > 0) {
      ...
      for (var post in posts) {
        allMarkers.add(new MapMarker(
                id: posts.indexOf(post).toString(),
                position: LatLng(post.lat, post.lon),
                icon: BitmapDescriptor.fromAsset("assets/default.png"),
                name: post.name,
                description: post.description,
                context: context,
                data: posts,
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: Text("Log In", style: TextStyle(color: Colors.green)), backgroundColor: Colors.white, actions: <Widget>[
          Center(
            child: ToggleButtons(
              children: <Widget>[
                Container(
                  child: Center(child: Text("All Posts", style: TextStyle(color: Colors.green))),
                  width: 70,
                ),
                Container(
                  child: Center(child: Text("Favorites", style: TextStyle(color: Colors.green))),
                  width: 70,
                )
              ],
              onPressed: (int index) {
      ...
      onPressed: _isButtonDisabled
          ...
          : () {
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (context) => new AllPostsScreen(posts, currentLocation),
                ),
              );
            },
    );
      ...
      onPressed: _isButtonDisabled
          ...
          : () {
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (context) => new FavoriteListScreen(posts, currentLocation),
                ),
              );
            },
    );
